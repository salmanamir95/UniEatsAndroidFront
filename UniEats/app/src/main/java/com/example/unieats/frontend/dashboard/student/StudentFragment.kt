...
class StudentFragment: Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val navHostFragment = childFragmentManager.findFragmentById(R.id.fragment_student_content_container) as NavHostFragment
        navController = navHostFragment.navController

        // 2. Initialize the start destination
        if (childFragmentManager.fragments.isEmpty()) {
            navController.navigate(R.id.homePageStudentFragment)
        }

        // 3. Load navbar after navigation setup
        childFragmentManager.beginTransaction()
            .replace(R.id.fragment_student_navbar_container, NavbarFragment())
            .commit()

        // 4. Restore user state properly
        //val args: StudentFragmentArgs by navArgs()
//        val navHostFragment = childFragmentManager.findFragmentById(R.id.fragment_student_content_container) as NavHostFragment
//        navController = navHostFragment.navController
//        appBarConfiguration = AppBarConfiguration(
//            setOf(
//                R.id.homePageStudentFragment,
//                R.id.menuFragment,
//                R.id.cartFragment,
//                R.id.ordersFragment
//            )
